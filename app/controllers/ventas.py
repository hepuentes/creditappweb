...
def index():
    ...
    saldo_pendiente_total = sum(
        v.saldo_pendiente
        for v in ventas
        if v.tipo == "credito" and v.saldo_pendiente is not None
    )
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            if not productos_seleccionados:
                ...
                return render_template(
                    "ventas/crear.html", form=form, productos=productos_disponibles
                )

            current_app.logger.info(
                f"Productos decodificados: {productos_seleccionados}"
            )
            ...
            for item in productos_seleccionados:
                ...
                if producto_db.stock < cantidad:
                    flash(
                        f"Stock insuficiente para {producto_db.nombre}. Disponible: {producto_db.stock}",
                        "danger",
                    )
                    db.session.rollback()
                    return render_template(
                ...
                # NUEVO: Calcular precio unitario dinámicamente basado en la cantidad
                try:
                    precio_unitario_calculado = producto_db.calcular_precio_unitario(
                        cantidad
                    )
                    current_app.logger.info(
                        f"Producto {producto_db.nombre}: cantidad={cantidad}, precio calculado=${precio_unitario_calculado}"
                    )

                    # Validar que el precio calculado sea válido
                    if (
                        precio_unitario_calculado is None
                        or precio_unitario_calculado <= 0
                    ):
                        flash(
                            f"Error en configuración de precios para {producto_db.nombre}. Contacte al administrador.",
                            "danger",
                        )
                        db.session.rollback()
                        return render_template(
                    ...
                    current_app.logger.error(
                        f"Error calculando precio para producto {producto_db.nombre}: {e}"
                    )
                    flash(
                        f"Error calculando precio para {producto_db.nombre}. Contacte al administrador.",
                        "danger",
                    )
                    db.session.rollback()
                    return render_template(
                ...
                # Crear detalle de venta con precio calculado dinámicamente
                subtotal = cantidad * precio_unitario_calculado
                ...
                # Log para auditoría
                current_app.logger.info(f"Detalle de venta agregado: {detalle}")
            ...
            if form.tipo.data == "credito":
                ...
                try:
                    ...
                    current_app.logger.error(
                        f"Error al registrar movimiento de caja: {e}"
                    )
            ...
            try:
                calcular_comision(
                    total_venta_calculado, current_user.id, nueva_venta.id
                )
            except Exception as e:
