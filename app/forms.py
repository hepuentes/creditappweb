...
class LoginForm(FlaskForm):
    email = StringField(
        "Email", validators=[DataRequired(), Email(message="Email inválido")]
    )
...
class ClienteForm(FlaskForm):
    ...
    email = StringField(
        "Email", validators=[Optional(), Email(message="Email inválido")]
    )
...
class ProductoForm(FlaskForm):
    ...
    precio_compra = FloatField(
        "Precio de Compra", validators=[Optional(), NumberRange(min=0)]
    )
    precio_venta = FloatField(
        "Precio de Venta", validators=[DataRequired(), NumberRange(min=0)]
    )
    stock = IntegerField("Stock", validators=[DataRequired(), NumberRange(min=0)])
    stock_minimo = IntegerField(
        "Stock Mínimo", validators=[DataRequired(), NumberRange(min=0)]
    )
    unidad = StringField("Unidad", validators=[Optional()])

    # NUEVOS CAMPOS PARA PRECIOS DIFERENCIADOS
    tiene_precio_individual = BooleanField(
        "Configurar precios diferenciados por cantidad"
    )
    precio_individual = FloatField(
        "Precio Individual (por unidad)",
        validators=[Optional(), NumberRange(min=0)],
        render_kw={"placeholder": "Ej: 5000"},
    )
    cantidad_kit = IntegerField(
        "Cantidad mínima para precio de kit",
        validators=[Optional(), NumberRange(min=2)],
        render_kw={"placeholder": "Ej: 3", "value": 3},
    )
    precio_kit = FloatField(
        "Precio unitario en kit",
        validators=[Optional(), NumberRange(min=0)],
        render_kw={"placeholder": "Ej: 4000"},
    )

    submit = SubmitField("Guardar Producto")

    def validate_tiene_precio_individual(self, field):
        """Validación personalizada para precios diferenciados"""
        if field.data:  # Si está activado el checkbox
            # Validar que todos los campos requeridos estén llenos
            if not self.precio_individual.data:
                raise ValidationError(
                    "El precio individual es requerido cuando se configuran precios diferenciados."
                )

            if not self.precio_kit.data:
                raise ValidationError(
                    "El precio de kit es requerido cuando se configuran precios diferenciados."
                )

            if not self.cantidad_kit.data or self.cantidad_kit.data < 2:
                raise ValidationError("La cantidad de kit debe ser mayor a 1.")

            # Validar lógica de negocio: precio individual > precio kit
            if self.precio_individual.data <= self.precio_kit.data:
                raise ValidationError(
                    "El precio individual debe ser mayor que el precio de kit."
                )

    def validate_precio_individual(self, field):
        """Validación específica para precio individual"""
        if self.tiene_precio_individual.data and field.data:
            if field.data <= 0:
                raise ValidationError("El precio individual debe ser mayor a cero.")

    def validate_precio_kit(self, field):
        """Validación específica para precio de kit"""
        if self.tiene_precio_individual.data and field.data:
            if field.data <= 0:
                raise ValidationError("El precio de kit debe ser mayor a cero.")

    def validate_cantidad_kit(self, field):
        """Validación específica para cantidad de kit"""
        if self.tiene_precio_individual.data and field.data:
            if field.data < 2:
                raise ValidationError("La cantidad de kit debe ser al menos 2.")
...
class AbonoForm(FlaskForm):
    cliente_id = SelectField(
        "Cliente", coerce=int, validators=[Optional()]
    )  # Cambiado a Optional
    tipo_credito = SelectField(
        "Tipo de Crédito",
    ...
    venta_id = SelectField(
        "Venta/Crédito", coerce=int, validators=[Optional()]
    )  # Cambiado a Optional
    credito_id = HiddenField("ID Crédito")
    credito_venta_id = HiddenField("ID Crédito Venta")
    monto = StringField(
        "Monto", validators=[Optional()]
    )  # Validación manual en controlador
    caja_id = SelectField(
        "Caja", coerce=int, validators=[Optional()]
    )  # Validación manual en controlador
    ...
    def validate_venta_id(self, field):
        """Validar que se seleccione una venta válida"""
...
class CreditoForm(FlaskForm):
    ...
    plazo = IntegerField(
        "Plazo (días)", validators=[DataRequired(), NumberRange(min=1)]
    )
...
class CajaForm(FlaskForm):
    ...
    saldo_inicial = FloatField(
        "Saldo Inicial",
        validators=[
            InputRequired(
                message="Este campo es obligatorio"
            ),  # Cambiado de DataRequired a InputRequired
...
class MovimientoCajaForm(FlaskForm):
    ...
    caja_destino_id = SelectField(
        "Caja Destino", coerce=lambda x: int(x) if x else None, validators=[Optional()]
    )
...
class ConfiguracionForm(FlaskForm):
    nombre_empresa = StringField(
        "Nombre de la Empresa", validators=[DataRequired(), Length(max=100)]
    )
    direccion = StringField("Dirección", validators=[Optional(), Length(max=200)])
    telefono = StringField("Teléfono", validators=[Optional(), Length(max=20)])
    logo = FileField("Logo de la Empresa")
    moneda = StringField(
        "Símbolo de Moneda", validators=[DataRequired(), Length(max=5)]
    )
    iva = FloatField(
        "IVA (%)", validators=[DataRequired(), NumberRange(min=0, max=100)]
    )
...
class AbonoEditForm(FlaskForm):
    monto = StringField(
        "Monto", validators=[DataRequired(message="El monto es obligatorio")]
    )
    caja_id = SelectField(
        "Caja",
        coerce=int,
        ...
        try:
            ...
            if self.abono and self.abono.venta:
                ...
                if monto_int > saldo_disponible:
                    raise ValidationError(
                        f"El monto no puede ser mayor al saldo disponible (${saldo_disponible:,.0f})"
                    )
